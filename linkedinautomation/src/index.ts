...
export default {
  async fetch(request: Request, env: Env) {
    ...
    const REDIRECT_URI = "https://linkedinautomation.pages.dev/auth/linkedin/callback";
    ...
    if (url.pathname === "/auth/linkedin") {
      ...
      linkedInAuthUrl.searchParams.set("redirect_uri", REDIRECT_URI);
    ...
    if (url.pathname === "/auth/linkedin/callback") {
      ...
      if (error) return new Response(`LinkedIn Error: ${error}`, { status: 400 });
      ...
      const tokenResponse = await fetch("https://www.linkedin.com/oauth/v2/accessToken", {
        ...
        body: new URLSearchParams({
          ...
          client_id: env.LINKEDIN_CLIENT_ID,       // Use env here
          client_secret: env.LINKEDIN_CLIENT_SECRET, // Use env here
        }),
      });
      ...
      // Step 4: Upsert token in Supabase
      await fetch(`${env.SUPABASE_URL}/rest/v1/linkedin_tokens?on_conflict=access_token`, {
        method: "POST",
        headers: {
